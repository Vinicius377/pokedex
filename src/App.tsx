...
function App() {
  ...
  const [page,setPage]=useState({offset:0,pageNumber:1})
  ...
  const req= async (action:string)=>{
    ...
    const response= await fetch(`https://pokeapi.co/api/v2/pokemon?limit=42&offset=${page.offset}`)
    const valores= await response.json()
    setPage(prev=>({offset:prev.offset+btnAction,
  ...
  return (
    <C.Container>
      <C.Area>
        <header>
          <button onClick={()=>page.offset>42 && req("prev")} className="btn">Prev</button>
          <h1>{page.pageNumber}</h1>
          <button onClick={()=>page.offset<1000 && req("next")} className="btn">Next</button>
        </header>
        <main>
        {poke?.map(pokemon=>{
